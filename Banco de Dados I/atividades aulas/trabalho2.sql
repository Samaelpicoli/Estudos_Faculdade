...
3) select concat (nome_funcionario, ' ', sobrenome) AS 'Nome Completo' from funcionarios;
   select concat (nome_cliente, ' ', sobrenome) AS 'Nome Completo' from clientes;

4) select concat ('Produto Mais Vendido: ',  descricao) from tipo where codigo = 1;

5) SELECT CONCAT('Quantidade no estoque: ', IFNULL(quantidade, 0)) FROM estoque WHERE codigo_estoque = 3;

6) SELECT CONCAT('Quantidade no estoque: ', quantidade)
COALESCE (NULL, quantidade, NULL, 0)
FROM estoque
WHERE codigo_estoque = 3;
