...
Atividades Fixação Trigger Aula 02/05

1) 
inseri a coluna preco_desconto (float) na tabela tipo
Create TRIGGER `tipo_desconto` BEFORE UPDATE ON `tipo` FOR EACH ROW 
BEGIN
	SET NEW.preco_desconto = NEW.preco * 0.90;
END
invocando:
UPDATE tipo SET preco = 71500.00 WHERE codigo = 2;

2) 
inseri a coluna salario_liquido (float) e salario_bruto (float) na tabela clientes
Create TRIGGER `clientes_salarios` BEFORE UPDATE ON `clientes` FOR EACH ROW 
BEGIN
	SET NEW.salario_bruto = NEW.salario_liquido - (NEW.salario_liquido * 0.10);
END
invocando:
UPDATE clientes SET salario_liquido = 3400.00 WHERE codigo = 5;
